...
void localization()
{
  ...
  for (int i = 0; i < NUM_BEACONS ; i++)
  {
    ...
    {
      ;   // Loop until BEACON_TIMEOUT_THRESHOLD so that we stay in sync witht the beacons pinging
    }
  }
...
void beaconRecvData()
{
  ...
  if (irrecv.decode(&irData))   // See if there is any IR data that has been decoded
  {
    ...
    if (beaconID > 0 && beaconID <= NUM_BEACONS)
    {
      ...
      #if DEBUG
      while (analogRead(usPin) >= voltReadMax)
      {
        voltReadMax = analogRead(usPin);    // Note that the analogRead operation takes around 100 microseconds to complete. Thus, the voltReadMax is only a very rough approximation of what the maximum might be
      }
      #endif
